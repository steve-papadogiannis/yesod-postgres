...
postPasswordR = do
    ...
    case maid of
        ...
        Just aid -> do
            ...
            if not needOld then confirmPassword aid jcreds else do
                ...
                case (mrealpass, current) of
                    (Nothing, _) ->
                        loginErrorMessage "You do not currently have a password set on your account"
                    (_, Nothing) ->
                        loginErrorMessageI Msg.BadSetPass
